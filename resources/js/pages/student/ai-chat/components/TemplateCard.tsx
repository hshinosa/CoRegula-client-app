...
export function TemplateCard({ template, onUse, onEdit, onDelete, isDeleting }: TemplateCardProps) {
    return (
        <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="group rounded-xl border border-[#E5E7EB]/50 bg-white/50 p-4 transition-all hover:border-[#88161c]/20 hover:bg-white/80"
        >
            <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                        <h3 className="text-sm font-semibold text-[#4A4A4A] truncate">{template.title}</h3>
                        {template.is_global && (
                    ...
                    {template.description && (
                        <p className="mt-1 text-xs text-[#6B7280] line-clamp-2">{template.description}</p>
                    )}
            ...
            <div className="mt-3 flex items-center gap-2 border-t border-[#F3F4F6] pt-3">
                <button
                    ...
                    className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-[#88161c]/10 px-3 py-2 text-xs font-medium text-[#88161c] transition-colors hover:bg-[#88161c]/20"
                >
                ...
                {!template.is_global && (
                    <>
                        <button
                            ...
                            className="rounded-lg p-2 text-[#6B7280] transition-colors hover:bg-[#F3F4F6] hover:text-[#4A4A4A]"
                            title="Edit template"
                        >
